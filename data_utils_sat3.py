...
def convert_standardizedtests(example, **kwargs):
    context = example['context']
    context = context if context.endswith('.') else context + '.'
    question = example['question']
    choice_names = ['answer'+item for item in ['A', 'B', 'C', 'D']]
    ...
    for choice in choices:
        ...
        ch_text = str(choice).strip().lower()
    ...
    label = ['A', 'B', 'C', 'D'].index(example['correct'])  
...
data_converters = {
    ...
    'standardizedtests': convert_standardizedtests,
...
def get_datasets(data_name, data_path='./external', data_config=None):
    ...
    if data_name == 'commonsenseqa':
        ...
        print(f"Expected data path for socialiqa: {data_path}")
        dev_path = '/scratch/smj490/alc-main/external/socialiqa/socialIQa_v1.4_dev.jsonl'
        test_path = '/scratch/smj490/alc-main/external/socialiqa/socialIQa_v1.4_tst.jsonl'
        train_path = '/scratch/smj490/alc-main/external/socialiqa/socialIQa_v1.4_trn.jsonl'
        datasets = {}
        datasets['dev'] = get_lines(dev_path)
        datasets['test'] = get_lines(test_path)
        datasets['train'] = get_lines(train_path)
    elif data_name == 'standardizedtests':
        #datasets['train'] = get_lines(os.path.join(dir_download_path, 'socialIQa_v1.4_trn.jsonl'))
        print(f"Expected data path for standardizedtests: {data_path}")
        dev_path = '/scratch/smj490/alc-main/external/standardizedtests/standardizedtests_v3_dev.jsonl'
        test_path = '/scratch/smj490/alc-main/external/standardizedtests/standardizedtests_v3_tst.jsonl'
        train_path = '/scratch/smj490/alc-main/external/standardizedtests/standardizedtests_v3_trn.jsonl'
        datasets = {}
        datasets['dev'] = get_lines(dev_path)
        datasets['test'] = get_lines(test_path)
        datasets['train'] = get_lines(train_path)
