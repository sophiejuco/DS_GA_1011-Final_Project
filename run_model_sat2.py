...
import data_utils_sat3
import model_utils_sat2
...
def main():
    ...
    model = model_class(model_name=args.model_name, cache_dir=args.cache_dir, load_model=True)
    mask_token = model.tokenizer.mask_token
    #D = data_utils_sat3.load_data(args.data_name, args.st_data_path, mask_token=mask_token, tokenizer=model.tokenizer, data_config=args.data_config)
    D = data_utils_sat3.load_data(args.data_name, mask_token=mask_token, tokenizer=model.tokenizer, data_config=args.data_config)
    ...
    for ep in range(args.fsl_n_episodes):
        ...
        if args.fsl_n_samples > 0:
            ...
            fsl_examples = [data_utils_sat3.format_labeled_example(D['converted'][args.fsl_sampling_split][idx]) for idx in fsl_train_indices]
        ...
        for e_idx in range(len(D_fsl['processed'][split])):
            try:
                example = D_fsl['processed'][split][e_idx]

                # Add saliency computation and result saving
                result = model.score_with_saliency(example)  # Compute scores and saliency
                #print(f"Keys in example: {example.keys()}")
                #print(f"Contents of example: {example}")
                save_results_with_saliency(example, result, output_file='results_with_saliency.json')

                print(f"Processed example {e_idx + 1}/{len(D_fsl['processed'][split])}")
            except Exception as e:
                print(f'Failed for {e_idx} with error {str(e)}')
                failures += 1

        print("Ran with", failures, "failures out of", len(D_fsl['processed'][split]), "total.")
...
def get_scores(args, model, data, split, cached_output_name=None, cache_exists=False):
    ...
    if not cache_exists:
        ...
        for rank in range(num_processes):
            ...
            if num_processes == 1:
                model_utils_sat2.score_fn(rank, model, D['processed'][split],
                                     p_indices, return_dict)
            else:
                p = mp.Process(target=model_utils_sat2.score_fn, args=(rank, model,
                                                  D['processed'][split],
                                                  p_indices, return_dict))
                p.start()
                processes.append(p)
...
def process_model_outputs(args, model, data, split, scores, results_file):
    ...
    for abl in ['answer_only', 'answer_only_worst']:
        preds = model_utils_sat2.predict(scores, mode=abl)[0]
    ...
    for abl in (['answer_only', 'answer_only_worst', 'answer_only_norm', 'answer_only_worst_norm', 'uncalibrated', 'length_normalized'] + 
        ...
        preds, preds_info = model_utils_sat2.predict(scores, mode=abl)
        saved_preds[abl] = preds
...
def save_results_with_saliency(example, result, output_file):
    """
    Save minimal saliency results to a file, avoiding large arrays.
    """
    def get_compact_score(score_dict):
        # Extract only essential scoring information
        return {
            'choice_idx': score_dict['choice_idx'],
            'score': float(score_dict['score']),
            'context_saliency': [
                (token, float(score)) 
                for token, score in score_dict['context_saliency']
            ],
            'choice_saliency': [
                (token, float(score))
                for token, score in score_dict['choice_saliency']
            ]
        }
    
    # Prepare minimal output data
    output_data = {
        'context': example['context'],
        'choices': example['choices'],
        'predicted_choice': int(np.argmin([s['score'] for s in result['saliency']])),
        'saliency_data': [
            get_compact_score(saliency) 
            for saliency in result['saliency']
        ]
    }
